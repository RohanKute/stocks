...
const buyStock = router.post('/buy', async (req, res) => {
    ...
    try {
        ...
        console.log(userAccBalance);
        const stocksList = await prisma.stocks.create({
            data : {
          ...
          const stocksL = await prisma.userAccount.findMany()
                // const stocksList = await prisma.stocks.fin
        
        console.table(stocksL);
